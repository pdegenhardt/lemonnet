...
/// <summary>
/// Preflow (push-relabel) maximum flow algorithm implementation.
/// This algorithm is generally faster than Edmonds-Karp with O(n²√m) complexity.
/// </summary>
public class Preflow : IDisposable
{
    ...
    private static extern long lemon_preflow(IntPtr graph, IntPtr capacity_map,
    ...
    /// <summary>
    /// Creates a new Preflow algorithm instance.
    /// </summary>
    ...
    /// <summary>
    /// Creates a new Preflow algorithm instance with a new capacity map.
    /// </summary>
    /// <param name="graph">The digraph to run the algorithm on.</param>
    ...
    public Preflow SetCapacity(Arc arc, long capacity)
    {
        ThrowIfDisposed();
    ...
    /// <summary>
    /// Runs the Preflow algorithm to find maximum flow.
    /// </summary>
    ...
    public MaxFlowResult Run(Node source, Node target)
    {
        ...
        try
        {
            long maxFlowValue = lemon_preflow(
                graph.Handle,
                capacityMap.Handle,
                source.Id,
                target.Id,
    ...
    private void ThrowIfDisposed()
    {
        if (disposed)
        {
            throw new ObjectDisposedException(nameof(Preflow));
        }
    }
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!disposed)
        {
            // Note: Preflow doesn't maintain any native state between calls,
            // so there's nothing to clean up here. The IDisposable implementation
            // is added for API consistency with EdmondsKarp and to prevent use after
